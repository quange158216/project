...
<table>
					<c:forEach var="notice" items="${ notice.noticesList }">
						<tr>
							...
							<td><a href="${ pageContext.request.contextPath }/notice/deleteNotice.action?nid=${ notice.nid }" onclick="confirm('确认删除！')">删除</a></td>
						</tr>
					</c:forEach>
				</table>
				...
				<center>
					...
					<c:if test="${ notice.totalPage ne 1 }">
						<a href="${ pageContext.request.contextPath }/notice/list.action?pc=${ 1 }&manager=1">首页</a>
					</c:if>
					<c:if test="${ notice.pageCode > 1 }">
						<a href="${ pageContext.request.contextPath }/notice/list.action?pc=${ notice.pageCode - 1 }&manager=1">上一页</a>
					</c:if>
					...
					<c:forEach var="i" begin="${ begain }" end="${ end }">
						<a href="${ pageContext.request.contextPath }/notice/list.action?pc=${ i }&manager=1">[${ i }]</a>
					</c:forEach>
					
					<c:if test="${ notice.pageCode < notice.totalPage }">
						<a href="${ pageContext.request.contextPath }/notice/list.action?pc=${ notice.pageCode + 1 }&manager=1">下一页</a>
					</c:if>
					<c:if test="${ notice.totalPage ne 1 }">
						<a href="${ pageContext.request.contextPath }/notice/list.action?pc=${ notice.totalPage }&manager=1">尾页</a>
					</c:if>
				</center>
</body>
</html>